...
export async function GET(req: Request) {
  ...
  // NOTE: only five params now; SQL uses $1..$5 (no gaps)
  const params: (string|number)[] = [mode, gcieid, custid, asOf, range];

  try {
    const r = await pg.query(LOWER_SQL, params);
    return NextResponse.json(buildPayload(r.rows, asOf, range));
  } catch (eLower:any) {
    try {
      const r2 = await pg.query(CAMEL_SQL, params);
      return NextResponse.json(buildPayload(r2.rows, asOf, range));
    } catch (eCamel:any) {
