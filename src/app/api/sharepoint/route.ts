...
export async function GET() {
  ...
  const userTenant = await prisma.userTenant.findFirst({ where: { userId: (session.user as any).id } });
...
export async function POST(req: Request) {
  ...
  const newNode = await prisma.sharePointNode.create({
    data: { name: name.trim(), parentId: parentId ?? null, tenantId: userTenant.tenantId, type, icon },
  });
