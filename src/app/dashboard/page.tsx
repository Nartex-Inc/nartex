...
// ===================================================================================
// Types de données & Système de design
// ===================================================================================
type SalesRecord = {
  salesRepName: string; customerName: string; itemCode: string; itemDescription: string; invoiceDate: string; salesValue: number;
};
type FilterState = {
  salesReps: string[]; itemCodes: string[]; customers: string[];
};

const GRADIENT_COLORS = [
  { start: "#3b82f6", end: "#1e40af" }, { start: "#8b5cf6", end: "#5b21b6" }, { start: "#06b6d4", end: "#0e7490" },
  { start: "#10b981", end: "#047857" }, { start: "#f59e0b", end: "#d97706" }, { start: "#ef4444", end: "#b91c1c" },
...
// ===================================================================================
// Composant de nombre animé haute performance
// ===================================================================================
...
const DashboardContent = () => {
  ...
  const handleSelect = (category: keyof FilterState, value: string, isShiftClick: boolean = false) => {
    setFilters(prev => {
      ...
      let newValues = isShiftClick ? (isSelected ? existing.filter(v => v !== value) : [...existing, value]) : (isSelected && existing.length === 1 ? [] : [value]);
      ...
      <div className="border-b border-gray-900 pb-8">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="flex items-center flex-wrap gap-4">
            <select value={stagedSelectedRep} onChange={(e) => setStagedSelectedRep(e.target.value)}
            ...
            <div className="flex items-center gap-2">
              <input type="date" value={stagedDateRange.start} onChange={(e) => setStagedDateRange(prev => ({ ...prev, start: e.target.value }))}
              ...
              <input type="date" value={stagedDateRange.end} onChange={(e) => setStagedDateRange(prev => ({ ...prev, end: e.target.value }))}
                className="bg-gray-900/50 border border-gray-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500"/>
            </div>
            <button onClick={applyFilters} className="px-4 py-3 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">
              Appliquer
            </button>
        ...
        <ChartContainer title="Évolution du chiffre d'affaires" className="col-span-12 lg:col-span-7 xl:col-span-8">
          <ResponsiveContainer width="100%" height={350}>
            <AreaChart data={salesByMonth}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="value" stroke="#3b82f6" strokeWidth={2} fillOpacity={1} fill="url(#salesGradient)" animationDuration={1000}/>
            </AreaChart>
          </ResponsiveContainer>
        </ChartContainer>
