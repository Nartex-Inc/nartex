      ...
      >
        <div
          className={cn(
            ...
            enabled ? "left-7 bg-[hsl(var(--accent))]" : "left-1 bg-white"
          )}
        />
      </div>
    </label>
...
export function PriceModal({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-2 md:p-4">
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-[98vw] max-h-[94vh] bg-[hsl(var(--bg-surface))] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-scale-in"
      >
        {/* Modal Header - Accent Banner */}
        <div className="flex-shrink-0 bg-[hsl(var(--accent))] px-4 md:px-6 py-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                {priceLists.map(pl => (
                  <option key={pl.priceId} value={pl.priceId} className="text-[hsl(var(--text-primary))] bg-[hsl(var(--bg-surface))]">
                    {pl.code} - {pl.name}
                  </option>
                ))}
              </select>
        ...
        {/* Modal Content */}
        <div className="flex-1 overflow-auto p-3 md:p-5 bg-[hsl(var(--bg-base))]">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-64 gap-4">
              <Loader2 className="w-10 h-10 animate-spin text-[hsl(var(--accent))]" />
              <p className="text-[hsl(var(--text-muted))] font-medium">Chargement des prix...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center h-64 gap-4">
              <div className="text-6xl text-[hsl(var(--danger))]">!</div>
              <p className="text-xl font-bold text-[hsl(var(--danger))]">Erreur</p>
              <p className="text-[hsl(var(--text-muted))] mt-1">{error}</p>
            </div>
          ) : itemsWithPrices.length > 0 ? (
            <div className="space-y-6">
              {groupByClass(itemsWithPrices).map(group => (
                <div key={group.className} className="rounded-xl overflow-hidden border border-[hsl(var(--border-default))]">
                  {/* Class Header */}
                  <div className="bg-[hsl(var(--accent))] px-4 py-3">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-[hsl(var(--bg-muted))]">
                          <th className="px-3 py-3 text-left font-bold text-[hsl(var(--text-secondary))] sticky left-0 bg-[hsl(var(--bg-muted))] z-10">
                            Article
                          </th>
                          <th className="px-3 py-3 text-center font-bold text-[hsl(var(--text-secondary))]">
                            CAISSE
                          </th>
                          <th className="px-3 py-3 text-center font-bold text-[hsl(var(--text-secondary))]">
                            Format
                          ...
                          <th className="px-3 py-3 text-right font-bold text-[hsl(var(--danger))]">
                            PDS
                          </th>
                        </tr>
                      </thead>
                        ...
                        {group.items.map((item, itemIdx) => (
                          item.ranges.map((range, rangeIdx) => (
                            ...
                            >
                              ...
                              <td className="px-3 py-2 text-center text-[hsl(var(--text-tertiary))]">
                                {rangeIdx === 0 && item.caisse}
                              </td>

                              <td className="px-3 py-2 text-center font-medium text-[hsl(var(--text-secondary))]">
                                ...
                                return (
                                  ...
                                  >
                                    {price !== undefined ? (
                                      ...
                                      <span className="text-[hsl(var(--text-muted))]">&mdash;</span>
                                    )}
                                  </td>
                                );
                              })}

                              <td className="px-3 py-2 text-right font-bold tabular-nums text-[hsl(var(--danger))]">
            ...
            <div className="flex flex-col items-center justify-center h-64 gap-4">
              <p className="text-xl font-bold text-[hsl(var(--text-muted))]">Aucun prix trouvé</p>
              <p className="text-[hsl(var(--text-muted))]">Essayez une autre sélection</p>
            </div>
          )}
        </div>

        {/* Modal Footer */}
        <div className="flex-shrink-0 px-4 md:px-6 py-3 bg-[hsl(var(--bg-muted))] border-t border-[hsl(var(--border-default))]">
