...
export const PipelineCard: React.FC<{
  ...
  return (
    // Main card container - THIS IS THE FIRST OPENING DIV
    <div className="flex flex-col rounded-xl border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      {/* Bottom section of the card for projects list */}
      <div className="flex-1 space-y-2 p-5 pt-3 border-t">
        {projects.length > 0 ? (
          projects.slice(0, 3).map((project) => (
            <Link key={project.id} href={`/dashboard/projects/${project.id}`} className="group block truncate">
              <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">{project.name}</p>
              <p className="text-xs text-muted-foreground">Demandeur: {project.initiator}</p>
            </Link>
          ))
        ) : (
    ...
    </div> // <-- THIS IS THE CORRESPONDING CLOSING DIV for the main container
  );
};
