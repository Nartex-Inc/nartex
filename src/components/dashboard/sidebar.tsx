...
interface SidebarProps { isOpen: boolean; isMobileOpen: boolean; toggleSidebar: () => void; closeMobileSidebar: () => void; }
...
const NavLink = ({ item, isSidebarOpen, closeMobileSidebar, isMobile }: { item: NavItem; isSidebarOpen: boolean; closeMobileSidebar: () => void; isMobile: boolean }) => {
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href={item.href} className={cn("flex h-10 w-10 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:text-foreground hover:bg-muted", isActive && "bg-primary/10 text-primary font-semibold")}>
  ...
  return (
    <Link href={item.href} onClick={handleLinkClick} className={cn("flex items-center gap-4 rounded-md px-4 py-2.5 text-muted-foreground transition-all hover:text-foreground hover:bg-muted", isActive && "bg-primary/10 text-primary font-semibold")}>
      <item.icon className="h-5 w-5" />
...
const NavGroup = ({ title, isSidebarOpen, children }: { title: string; children: React.ReactNode; isSidebarOpen: boolean; }) => (
  <div>
    {isSidebarOpen && (<h2 className="px-4 pt-2 pb-1 text-xs font-semibold tracking-wider text-muted-foreground/60 uppercase">{title}</h2>)}
    <nav className={cn("flex flex-col gap-1", !isSidebarOpen && "items-center")}>{children}</nav>
  </div>
);
...
export function Sidebar({ isOpen, isMobileOpen, toggleSidebar, closeMobileSidebar }: SidebarProps) {
  ...
  useEffect(() => {
    fetch('/api/user/tenants').then((res) => res.json()).then((data) => {
        setTenants(data);
        const storedTenantId = localStorage.getItem('currentTenantId');
        if (storedTenantId && data.some((t: {id: string}) => t.id === storedTenantId)) { setCurrentTenantId(storedTenantId); }
        else if (data.length > 0) { const firstId = data[0].id; setCurrentTenantId(firstId); localStorage.setItem('currentTenantId', firstId); }
      }).catch((error) => console.error('Failed to fetch tenants:', error));
  }, []);
  const handleTenantChange = (value: string) => { setCurrentTenantId(value); localStorage.setItem('currentTenantId', value); router.refresh(); };
  const handleLogout = () => signOut({ callbackUrl: "/" });

  return (
    <aside className={cn("flex-col transition-all duration-300 ease-in-out bg-card border-r", "fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full lg:relative lg:translate-x-0", isMobileOpen && "translate-x-0", isOpen ? "lg:w-72" : "lg:w-20")}>
          ...
          <div className="flex flex-col gap-6 px-2">
            {navItemGroups.map((group) => (
              <NavGroup key={group.title} title={group.title} isSidebarOpen={isExpanded}>
                {group.items.map((item) => (<NavLink key={item.href} item={item} isSidebarOpen={isExpanded} closeMobileSidebar={closeMobileSidebar} isMobile={isMobileOpen} />))}
              </NavGroup>
            ))}
          </div>
        </div>
